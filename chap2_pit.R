...
ggplot(data=san_pit_figs) + geom_histogram(aes(x=hours), bins=24) +  facet_wrap(~ year)
#All years, detections by hour per species 
ggplot(data=subset(san_pit_figs, !is.na(species))) + geom_histogram(aes(x=hours), bins=24) +  facet_wrap(~ species)
#Detections per hour by species+year
ggplot(data=subset(san_pit_figs, !is.na(species))) + geom_histogram(aes(x=hours), bins=24) +  facet_wrap(~species ~year)

####Histograms for site####

#Number of detections at each site per year 
ggplot(data=subset(san_pit_figs, !is.na(site))) + geom_histogram(aes(x=site), bins=10) +facet_wrap(~year) 
#Number of detections at each site per species
ggplot(data=subset(san_pit_figs, !is.na(species), !is.na(site))) + geom_histogram(aes(x=site), bins=10) +facet_wrap(~species) 
#Number of detectons at each site per species+year
ggplot(data=subset(san_pit_figs, !is.na(species), !is.na(site))) + geom_histogram(aes(x=site), bins=10) +facet_wrap(~species ~year) 
...
ggplot(data=subset(san_pit_figs, !is.na(species), !is.na(hits))) + geom_histogram(aes(x=hits), bins=1000) 
